 ...
 */
  ...
  */
 function cdn_adventure_posted_on() {
  ...
  */
 function cdn_adventure_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function cdn_adventure_comment_count() {
  ...
  */
 function cdn_adventure_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && cdn_adventure_categorized_blog() ) {
  ...
  * @return bool
  */
 function cdn_adventure_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'cdn_adventure_categories' ) ) ) {
 		...
 		set_transient( 'cdn_adventure_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so cdn_adventure_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so cdn_adventure_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in cdn_adventure_categorized_blog.
  */
 function cdn_adventure_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'cdn_adventure_categories' );
 }
 add_action( 'edit_category', 'cdn_adventure_category_transient_flusher' );
 add_action( 'save_post',     'cdn_adventure_category_transient_flusher' );
  ...
  */
 function cdn_adventure_comment_list( $comment, $args, $depth ) {
  ...
  */
 function cdn_adventure_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
